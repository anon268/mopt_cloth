...
class NormalizedEnv(object):
    ...
    def step(self, action, **kwargs):
        ...
        elif isinstance(self._wrapped_env.action_space, tuple) and self._wrapped_env.action_space == 0:
            lb, ub = self._wrapped_env.action_space.low, self._wrapped_env.action_space.high
            scaled_action = action.copy()
            if action[-1] < 0:
                scaled_action[:2] = lb[:2] + (action[:2] + 1.) * 0.5 * (ub[:2] - lb[:2])
                if self._clip:
                    scaled_action[:2] = np.clip(scaled_action[:2], lb[:2], ub[:2])
            scaled_action[3:5] = lb[2:] + (action[3:5] + 1.) * 0.5 * (ub[2:] - lb[2:])
            if self._clip:
                scaled_action[3:5] = np.clip(scaled_action[3:5], lb[2:], ub[2:])
        else:
