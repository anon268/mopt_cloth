...
from softgym.action_space.action_space import  Picker, PickerPickPlace, PickerQPG, PickAndPlace, PickAndPlacePos
...
class ClothEnv(FlexEnv):
    def __init__(self, observation_mode, action_mode, num_picker=2, render_mode='particle', picker_radius=0.025, picker_threshold=0.005, particle_radius=0.00625, **kwargs):
        ...
        assert action_mode in ['picker', 'pickerpickplace', 'sawyer', 'franka', 'picker_qpg', 'pickandplace', 'pickandplacepos']
        ...
        if action_mode == 'picker':
            ...
            cam_pos, cam_angle = self.get_camera_params()
            self.action_tool = PickAndPlacePos((self.camera_height, self.camera_height), cam_pos, cam_angle,
                                               picker_threshold=picker_threshold + 0.025,
                                               num_picker=num_picker, particle_radius=particle_radius, env=self,
                                               picker_low=(-0.5, 0., -0.5), picker_high=(0.5, 0.3, 0.5)
                                              )
