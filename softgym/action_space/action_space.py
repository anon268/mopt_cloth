...
class Picker(ActionToolBase):
    def __init__(
        ...
        **kwargs
        ...
        self.picker_low, self.picker_high = np.array(list(picker_low)), np.array(
            list(picker_high)
        )
        ...
        self.spring_coef = (
            spring_coef  # Prevent picker to drag two particles too far away
        )

        space_low = (
            np.array([-0.1, -0.1, -0.1, 0] * self.num_picker) * 0.1
        )  # [dx, dy, dz, [0, 1]]
    ...
    def step(self, action):
        ...
        for i in range(self.num_picker):
            new_picker_pos[i, :] = self._apply_picker_boundary(
                picker_pos[i, :] + action[i, :3]
            )
            if pick_flag[i]:
                if (
...
class PickerPickPlace(Picker):
    def __init__(
        self, num_picker, env=None, picker_low=None, picker_high=None, **kwargs
    ):
        super().__init__(
        ...
        self.action_space = Box(
            np.array([*picker_low, 0.0] * self.num_picker),
            np.array([*picker_high, 1.0] * self.num_picker),
            dtype=np.float32,
        )
    ...
    def step(self, action):
        ...
        for i in range(
            ...
            if self.first_pick[0] is None:
                self.first_pick = self.picked_particles
            pyflex.step()
            total_steps += 1
        ...
        # if (i == 499):
        #     print(i, action)
        self.env.picked_id = self.first_pick[0]
        return total_steps
...
class PickerQPG(PickerPickPlace):
    def __init__(self, image_size, cam_pos, cam_angle, full=True, **kwargs):
        ...
        self.action_space = Box(
            np.array([-1.0, -1, -0.3, 0.0, -0.3]),
            np.array([1.0, 1.0, *([0.3] * 3)]),
            dtype=np.float32,
        )
    ...
    def _get_world_coor_from_image(self, u, v):
        ...
        depth = self._get_depth(
            matrix, vec, self.particle_radius
        )  # Height to be the particle radius
...
class PickAndPlace(PickerQPG):
    def __init__(self, image_size, cam_pos, cam_angle, full=True, **kwargs):
        ...
        space_low = np.array(
            [-1.0, -1.0, -1.0, -1.0] * self.num_picker
        )  # [u1, v1, u2, v2]
        space_high = np.array([1.0, 1.0, 1.0, 1.0] * self.num_picker)
        self.action_space = Box(space_low, space_high, dtype=np.float32)
    ...
    def step(self, action):
        ...
        u1, v1 = action[:2]
        u1 = ((u1 + 1.0) * 0.5) * self.image_size[0]
        v1 = ((v1 + 1.0) * 0.5) * self.image_size[1]
        ...
        u2, v2 = action[2:]
        u2 = ((u2 + 1.0) * 0.5) * self.image_size[0]
        v2 = ((v2 + 1.0) * 0.5) * self.image_size[1]
    ...
    def hide(self):
        hide_place = np.array([1.0, 0.5, 1.0, 0])
